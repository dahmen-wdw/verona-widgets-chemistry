...
Patched by:
- Alexander Dahmen (dahmen@werdenktwas.de), fixed Angular compatibility with document.baseURI URLs
...
function replaceUrlInCss(jsString, cssPath) {
  const regexUrl = /(?!new\s+)\burl\([^)]+\)/gi; // rebuild regex
  return jsString.replace(regexUrl, a => {
    ...
    // FIXME: Also, URLs containing 'document.baseURI' should not be replaced!
    if (a.search(/[~"#%*<>?{}|]/) > -1 || a.includes('document.baseURI')) {
      return a;
    }
    const regexFile = /\((.*?)\)/ig;
